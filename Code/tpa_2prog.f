...
c
c--------------------------------------------------------------------------
      parameter (nx=1000,ny=1000,nsupr=4,nmax=nx*ny*(2+nsupr),itmax=3)
c--------------------------------------------------------------------------
c
...
c        cycle time = dcymin
c--------------------------------------------------------------------------
      parameter (dlrmin=0.12d0,dlrmax=0.17d0,dcymin=2.3d0,dcymax=3.3d0,
     +     nstep=11)
c--------------------------------------------------------------------------
c
...
c--------------------------------------------------------------------------
      parameter (nx=1000,ny=1000,nsupr=4,nmax=nx*ny*(2+nsupr),itmax=3,
     +   dratio=0.58d0,dfac=dratio/2.0d0/(1.0d0-dratio))
c--------------------------------------------------------------------------
c
c     model parameters
c
...
c                  if (random.lt.dfac) then
c                     nclsz(ncrypt(lx,ly,2+ns),2,i1)=
c     +                  nclsz(ncrypt(lx,ly,2+ns),2,i1)+1
c                  endif
c---- suprabasal component - introduce some randomization
c                  call random_number(random)
c                  ns1=2*int(dble((nsupr+2)/2)*random)
c                  if (ns1.eq.0) goto 380
c                  do 340 ns=1,ns1
c                  nclsz(ncrypt(lx,ly,2+ns),2,i1)=
c     +              nclsz(ncrypt(lx,ly,2+ns),2,i1)+1
 340              continue
