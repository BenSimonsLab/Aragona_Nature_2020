...
c
c     The model involves a one-progenitor compartment model
...
c
c--------------------------------------------------------------------------
      parameter (nx=1000,ny=1000,nsupr=4,nmax=nx*ny*(2+nsupr),itmax=6)
c--------------------------------------------------------------------------
c
...
c        cycle time = dcymin
c--------------------------------------------------------------------------
      parameter (dlrmin=0.5d0,dlrmax=1.0d0,dcymin=3.5d0,dcymax=6.5d0,
     +     nstep=10)
c--------------------------------------------------------------------------
c
...
c---- output least-square distribution
 ...
 800     continue
     ...
     1   dsq(nstep,nstep),'}},'
      write(19,'(a120)') 'LabelStyle -> {FontFamily -> "Myriad Pro", 
...
c
c--------------------------------------------------------------------------
      parameter (dexpan=1.0d0,dprob=1.0d0/dexpan,dtoff=-3.0d0,
     +   dind=1.0d0)
c--------------------------------------------------------------------------
c
c     Model arrays
...
c---- no cell present
 ...
 106     continue
      ...
      write(*,*) 'starting clone number=',nclst,'out of ',nx*ny,' sites'
c-----------------------------------------------------------------------
...
c---- loop over time points
            do 310 nstep=itime(i1-1),itime(i1)
...
c---- count the number of sites that are occupied
               if (ncrypt(lx,ly,1).ne.-1) nsites(i1)=nsites(i1)+1
               if (ncrypt(lx,ly,2).ne.-1) nsites(i1)=nsites(i1)+1
c---- nclsz(x,1,y) basal size of clone x at time y
               nclsz(ncrypt(lx,ly,1),1,i1)=
     +            nclsz(ncrypt(lx,ly,1),1,i1)+1
c---- nclsz(x,2,y) suprabasal size of clone x at time y
               nclsz(ncrypt(lx,ly,2),2,i1)=
     +            nclsz(ncrypt(lx,ly,2),2,i1)+1
 380           continue
 390        continue
 400     continue
c-----------------------------------------------------------------------
...
c---- read in experimental data
      open(unit=18,file='ctrldist.dat')
c---- first timings
